...
export default function Membership() {
  ...
  const handleIndividualMembershipPurchase = async () => {
    try {
      ...
      const [securityDeposit, adultMembership] = await Promise.all([
        fetchProductBySlug('refundable-security-deposit'),
        fetchProductBySlug('single-adult-new-1-year')
      ])

      if (!securityDeposit || !adultMembership) {
        throw new Error('Could not fetch required products from WooCommerce')
      }
  ...
  const handleSeniorMembershipPurchase = async () => {
    try {
      ...
      // Fetch real products from WooCommerce
      const [securityDeposit, seniorMembership] = await Promise.all([
        fetchProductBySlug('refundable-security-deposit'),
        fetchProductBySlug('senior-citizen-new-1-year')
      ])

      if (!securityDeposit || !seniorMembership) {
        throw new Error('Could not fetch required products from WooCommerce')
      }
  ...
  const handleChildMembershipPurchase = async () => {
    try {
      ...
      // Fetch real products from WooCommerce
      const [securityDeposit, childMembership] = await Promise.all([
        fetchProductBySlug('refundable-security-deposit'),
        fetchProductBySlug('child-new-1-year')
      ])

      if (!securityDeposit || !childMembership) {
        throw new Error('Could not fetch required products from WooCommerce')
      }
  ...
  const handleFamilyMembershipPurchase = async () => {
    try {
      ...
      // Fetch real products from WooCommerce
      const [securityDeposit, familyMembership] = await Promise.all([
        fetchProductBySlug('refundable-security-deposit'),
        fetchProductBySlug('family-new-1-year')
      ])

      if (!securityDeposit || !familyMembership) {
        throw new Error('Could not fetch required products from WooCommerce')
      }
